...
(function (global, $) {
    ...
    BodyView.prototype.render = function (options, forceOrientation) {
        ...
        if (!needAllClear && container.isDisplayDataRow(position.row)) {
            ...
            if (requireHeight) {
                ...
                // case scroll up
                    ...
                    if (targetIndex > 0) {
                        var removeStart = container.rows.slice(targetIndex).reduce(function (init, value, index) {
                            if (init.height < self.viewHeight) {
                                var row = self.cache.rows[value.rowid];
                                init.height += (row.height || row.temporaryHeight || container.medianHeight);
                                init.index = index;
        ...
        if (needAllClear) {
            ...
            container.setContainerOffset(Math.max(options.scrollTop - self.viewHeight + position.offset, 0));
        }
